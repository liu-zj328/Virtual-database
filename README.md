...
Output File
Identification_match_result.xlsx
An Excel file generated in the same folder. It contains one row for each unique combination of Compound, PrecursorMZ, and Retention time (min) that had at least one database match (ppm ≤ 5). If no match is found, the file is not created and a message is printed to the console.
